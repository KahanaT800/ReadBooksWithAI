...
- 如果执行流到达函数体末尾的 `}` 而没有遇到 `return`，函数也会返回——但如果函数不是 `void` 类型，返回值是**未定义的**（垃圾值）。
...
注意 `find_int` 返回的是数组元素的地址。这是安全的，因为 `data` 数组在 `main` 的栈帧中，调用 `find_int` 时 `main` 的栈帧还在。但如果返回的是函数**自己的**局部变量的地址，就会出大问题——见下面的易错点。
...
C 没有"传引用"（pass by reference）机制。要让函数能修改调用者的变量，方法是**传入变量的地址**（即指针），函数通过解引用来修改原变量。
