...
指针可以加上或减去一个整数，结果仍然是指针。关键在于：**加的不是字节数，而是元素个数**。
...
缩放是 C 指针设计的核心。没有缩放，你每次操作指针都要手动计算字节偏移，既繁琐又容易出错。有了缩放，你可以用 `p++` 遍历任何类型的数组，编译器自动处理字节偏移。
...
**标准限制**：指针加减整数只能用于**指向数组元素**的指针（包括 `malloc` 分配的连续内存）。对一个普通标量变量的指针做 `p + 2` 是未定义行为——编译器不会报错，但结果没有意义。
...
注意循环中的 `&values[N]`：对一个有 N 个元素的数组，下标 0 到 N-1 是合法元素，`&values[N]` 指向最后一个元素之后的位置。这叫 **past-the-end 指针**——你可以获取它的地址、可以用它做比较，但**不能对它解引用**（因为那个位置不属于数组）。
