...
这不是语法强制的，但这是 C 社区广泛遵循的约定。把预处理指令（preprocessor directive）放最前面、函数原型放在 `main` 之前、函数定义放在 `main` 之后，是最常见的组织方式。
...
- 函数原型放在 `main` 之前，让编译器（compiler）在看到函数调用时已知道参数和返回值类型，能做类型检查
- 多文件项目中，这种结构直接对应"头文件（header file）放声明、源文件放定义"的工程规范
...
| 汇编 | 汇编器 | `.s` | `.o` | 把汇编代码翻译成机器指令（目标文件，object file） |
| 链接 | 链接器（linker） | `.o` + 库 | 可执行文件 | 把多个目标文件和库函数合并，解析符号引用 |
...
- 理解预处理阶段做了什么，才能理解为什么 `#include` 是"复制粘贴"、为什么宏（macro）没有类型检查
...
- 声明和初始化规则影响变量的作用域（scope）和生命周期（后面三节详讲）
...
> ➕ **字符串字面量（string literal）是"指向常量字符的指针"**
>
> 当你写 `"Hello"` 时，编译器把这 6 个字节（5 个字符 + NUL）存在只读区。表达式 `"Hello"` 的类型是 `char *`（历史原因），但**修改字符串字面量是未定义行为（undefined behavior, UB）**：
>
> ```c
...
标识符（identifier）的作用域（scope）是它在源代码中可以被使用的区域。编译器识别四种作用域：
...
- 作用域是理解链接属性（linkage）和存储类型（storage class）的前提——三者共同决定变量的可见性和生命周期
...
| **none** | 每次声明都是独立个体 | 局部变量（local variable）、函数参数 |
...
- 不知道为什么两个文件定义同名全局变量（global variable）会冲突
...
| 寄存器 | `register` | 寄存器（建议） | 同自动变量（automatic variable） | **不确定** |
