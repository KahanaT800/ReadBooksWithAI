...
- 理解"副作用"是后续学习操作符求值顺序和未定义行为（undefined behavior, UB）的基础——当多个副作用在同一表达式中出现且顺序不确定时，就是 UB
...
- 整型溢出（overflow）在有符号类型中是未定义行为，在无符号类型中是回绕（wrap around）
...
`typedef` 为已有类型创建一个新名字（类型别名，type alias）。语法和变量声明完全相同，只是前面加上 `typedef` 关键字：
